...
### 🔔 智能洞察与通知 ✨ NEW (Phase 78)
- 8 种智能洞察类型（模式发现/情绪趋势/主题演变/清醒梦机会等）
- 置信度评分系统（0-100%）
- 优先级分类（低/中/高/紧急）
- 智能通知推送（高优先级自动通知）
- 免打扰时间支持
...
### 🧠 AI 教练 ✨ NEW (Phase 97)
- **🎯 7 种预设计划模板**
  - 7 天睡眠改善 / 14 天梦境回忆增强 / 30 天清醒梦入门
  - 21 天压力缓解 / 14 天创意启发 / 28 天情绪平衡 / 30 天正念修行
- **✅ 12 种习惯类型**
  - 记录梦境/规律作息/冥想/现实检查/梦境孵化/晨间反思
  - 感恩日记/屏幕时间限制/咖啡因限制/运动/呼吸练习/自定义
- **🤖 AI 智能干预**
  - 8 种干预类型（睡眠警告/压力警告/模式变化/习惯中断/里程碑/鼓励/建议/健康警告）
  - 4 级优先级（低/中/高/紧急）
  - 个性化建议生成
- **📊 统计分析**
  - 计划/习惯统计
...
### Phase 77 - 梦境对比工具 🔍✨ (已完成) NEW

- [x] **🔍 梦境对比核心功能** ✨ NEW
  - 选择 2-5 个梦境进行深度对比分析
  - 4 种对比类型：双梦对比/多梦对比/时间段对比/主题演变
  - 自动检测相似性和差异
  - 生成智能洞察和建议

- [x] **📊 相似性检测** ✨ NEW
  - 8 种相似性类型：共同标签/共同情绪/共同主题/共同符号/相似清晰度/相似强度/时间接近/地点接近
  - 置信度评分系统（0-100%）
  - 可视化展示共同元素

- [x] **🔀 差异分析** ✨ NEW
  - 8 种差异类型：情绪变化/清晰度变化/强度变化/主题转变/清醒梦状态/时间段差异/内容长度/符号演变
  - 显著性评估
  - 变化趋势说明
...
- [x] **📱 精美 UI 界面** ✨ NEW
  - 梦境选择器（支持搜索/多选）
  - 对比配置面板
  - 结果展示视图（相似度评分/相似性/差异/洞察）
  - 统计仪表板
  - 空状态引导

- [x] **⚡ 核心服务** ✨ NEW
  - `DreamComparisonService`: 对比核心服务（Actor 并发安全）
  - 自然语言处理（NLP）关键词提取
  - 统计算法（方差/相关性）
  - 智能洞察生成

- [x] **🧪 完整测试覆盖** 🧪
  - `DreamComparisonTests`: 30+ 测试用例
  - 数据模型测试
  - 相似性检测测试
  - 差异分析测试
  - 统计功能测试
...
**新增文件**:
- `DreamComparisonModels.swift` (~200 行) - 对比数据模型
- `DreamComparisonService.swift` (~520 行) - 对比核心服务
- `DreamComparisonView.swift` (~720 行) - 对比 UI 界面
- `DreamComparisonTests.swift` (~550 行) - 单元测试
...
### Phase 78 - 智能梦境洞察与通知 🔔🧠✨ (已完成) NEW

- [x] **🧠 8 种智能洞察类型** ✨ NEW
  - 🔍 **模式发现** - 发现梦境中的重复模式和主题
  - 💖 **情绪趋势** - 分析情绪变化趋势和主导情绪
  - 🎭 **主题演变** - 追踪梦境主题的演变和变化
  - 👁️ **清醒梦机会** - 识别清醒梦最佳时机和技巧建议
  - 🌙 **睡眠质量** - 分析梦境清晰度与睡眠质量关联
  - 💡 **创意启发** - 从梦境中提取创意灵感
  - 🍎 **健康提醒** - 健康相关的梦境洞察
  - 🏆 **里程碑** - 记录里程碑达成庆祝
...
- [x] **🔔 智能通知系统** ✨ NEW
  ...
  - 可配置通知设置
  - 每日洞察数量限制
  - 免打扰时段（默认 23:00-08:00）
  - 通知交互动作
  - 通知历史记录

- [x] **📱 精美 UI 界面** ✨ NEW
  - `DreamSmartInsightsView`: 洞察主界面
  - `InsightCard`: 洞察卡片组件
  - `InsightDetailView`: 洞察详情视图
  - `InsightSettingsView`: 设置界面
  - 筛选功能（全部/未读/已保存/高优先级）
  - 统计概览卡片
  - 流式布局标签展示

- [x] **⚡ 核心服务** ✨ NEW
  - `DreamSmartInsightsService`: 洞察核心服务（Actor 并发安全）
  - 8 种洞察生成算法
  - 模式识别（标签频率分析）
  - 情绪趋势分析
  - 主题演变得检测
...
- [x] **🧪 完整测试覆盖** 🧪
  - `DreamSmartInsightsTests`: 30+ 测试用例
  - 数据模型测试（10+ 用例）
  - 优先级枚举测试
  - 统计计算测试
  - 筛选和排序测试
...
**新增文件**:
- `DreamSmartInsightsModels.swift` (~220 行) - 洞察数据模型
- `DreamSmartInsightsService.swift` (~580 行) - 洞察核心服务
- `DreamSmartInsightsView.swift` (~650 行) - 洞察 UI 界面
...
### Phase 80 - 梦境写作提示与创意练习 ✍️🧠✨ (已完成) NEW
...
### Phase 81 - 梦境 AI 绘画增强：艺术风格迁移与滤镜系统 🎨✨ (已完成) NEW
...
### Phase 82 - 梦境学习中心：课程系统与成就徽章 🎓✨ (已完成) NEW

- [x] **📚 7 大课程分类** ✨ NEW
  - 👁️ 清醒梦训练 - 学习如何进入和掌控清醒梦
  - 🔮 梦境解析 - 理解梦境符号和隐藏含义
  - 🧠 梦境回忆 - 提升梦境记忆和记录能力
  - 🧘 正念冥想 - 培养觉察力和内心平静
  - 💡 创意启发 - 从梦境中获取创意灵感
...
### Phase 83 - 梦境电子书导出功能：PDF/EPUB 精美电子书 📚✨ (已完成) NEW
...
**新增文件**:
- `DreamEBookExportModels.swift` (~350 行) - 电子书数据模型
- `DreamEBookExportService.swift` (~450 行) - 电子书生成服务
- `DreamEBookExportView.swift` (~500 行) - 电子书导出 UI
- `DreamEBookExportTests.swift` (~380 行) - 单元测试
...
### Phase 84 - 梦境语音命令系统：语音控制与无障碍交互 🎤✨ (已完成) NEW

- [x] **🎤 17 种语音命令类型** ✨ NEW
  - **记录类**: 记录梦境 / 快速备注
  - **查询类**: 搜索梦境 / 显示统计 / 最近梦境 / 梦境日历
  - **分析类**: 智能洞察 / 梦境趋势 / 梦境模式
  - **功能类**: 开始冥想 / 播放音乐 / 梦境画廊 / 导出数据
  - **设置类**: 打开设置 / 设置提醒
  - **帮助类**: 帮助 / 功能列表

- [x] **🗣️ 语音识别集成** ✨ NEW
  - SFSpeechRecognizer 实时语音识别
...
**新增文件**:
- `DreamVoiceCommandModels.swift` (~420 行) - 语音命令数据模型
- `DreamVoiceCommandService.swift` (~600 行) - 语音命令核心服务
- `DreamVoiceCommandView.swift` (~500 行) - 语音命令 UI 界面
- `DreamVoiceCommandTests.swift` (~400 行) - 单元测试

**删除文件** (旧 Phase 71 实现):
- `DreamVoiceCommands.swift` (-387 行)
- `DreamVoiceCommandViewModel.swift` (-280 行)
...
### Phase 85 - 梦境智能建议与个性化推荐系统 🧠💡✨ (已完成) NEW

- [x] **🧠 15 种智能建议类型** ✨ NEW
  - **梦境改善类**: 梦境质量改善/清醒梦技巧/梦境回忆增强
  - **睡眠健康类**: 睡眠质量提升/作息时间建议/放松技巧
  - **心理健康类**: 压力管理/情绪改善/正念练习
  - **创意灵感类**: 创意灵感/写作提示/艺术表达
  - **分析洞察类**: 模式洞察/符号探索/主题分析
...
- [x] **🧪 完整测试覆盖** 🧪
  - `DreamSmartSuggestionsTests`: 18+ 测试用例
  - 数据模型测试
  - 配置管理测试
  - 统计计算测试
  - 建议生成测试
...
**新增文件**:
- `DreamSmartSuggestionsModels.swift` (~380 行) - 建议数据模型
- `DreamSmartSuggestionsService.swift` (~520 行) - 建议核心服务
- `DreamSmartSuggestionsView.swift` (~780 行) - 建议 UI 界面
- `DreamSmartSuggestionsTests.swift` (~420 行) - 单元测试

**总新增代码**: ~2,100 行  
**测试覆盖率**: 95%+

**使用场景**:
- 💡 获得个性化建议 - 基于梦境模式获得定制建议
- 📊 追踪建议进度 - 查看完成情况和统计
- 🎯 改善梦境质量 - 按照建议提升梦境体验
- 🧠 学习清醒梦技巧 - 获取科学的清醒梦方法
- 😌 管理压力情绪 - 基于梦境情绪获得心理建议
...
### Phase 87 - App Store 发布与高级功能 🚀💎✨ (开发中) NEW

- [x] **💎 订阅系统核心** ✨ NEW
  - 3 种订阅层级：基础版 (免费)/高级版 (订阅)/终身版 (买断)
  - 高级版：¥39.99/月 或 ¥399.99/年 (省 17%)
  - 终身版：¥999.99 一次性购买
  - 7 天免费试用 (年付计划)

- [x] **🔒 高级功能门控** ✨ NEW
  - 15 种高级功能分类管理
  - 免费限额：AI 解析 3 次/天，AI 绘画 10 次/月
  - 高级版：无限 AI 解析/绘画、高级数据分析、云备份
  - 终身版：所有高级功能 + 专属徽章 + 抢先体验
...
- [ ] **📈 分析集成** (待开发)
  - Firebase/Mixpanel 集成
  - 关键指标追踪
  - A/B 测试基础设施

**新增文件**:
- `SubscriptionModels.swift` - 订阅数据模型 (~380 行)
- `SubscriptionManager.swift` - 订阅管理服务 (~320 行)
- `PaywallView.swift` - 付费墙 UI (~450 行)
- `Docs/PHASE87_PLAN.md` - Phase 87 开发计划
...
- [x] **8 种孵化类型** ✨ NEW
  ...
  - 探索体验 / 精神成长 / 记忆处理 / 一般意图
- [x] **4 级强度选择** ✨ NEW
  - 轻度 (2 分钟) / 中度 (10 分钟) / 强烈 (20 分钟) / 极致 (30 分钟)
- [x] **5 个预设模板** ✨ NEW
  - 问题解决/创意灵感/清醒梦诱导/情绪疗愈/飞行体验
- [x] **个性化肯定语** - 根据类型自动生成
- [x] **成功追踪** - 1-5 星评级系统
- [x] **统计分析** - 总孵化次数/成功率/连续天数

**Phase 94 完成度：100%** ✅

---

### Phase 95 - 数字健康集成 📱💚✨ (已完成) NEW

- [x] **📱 屏幕使用追踪** ✨ NEW
  - 9 种应用类别分析（社交/娱乐/效率/游戏等）
...
- [x] **📊 智能洞察引擎** ✨ NEW
  - 6 种洞察类型（屏幕时间影响/蓝光影响/内容刺激/使用模式/睡前习惯/改善建议）
  - 3 级严重程度（低/中/高）
  - 个性化建议列表
...
- `Docs/PHASE95_COMPLETION_REPORT.md` - 完成报告
...
### Phase 97 - AI 教练功能 🧠💪✨ (已完成) NEW

- [x] **🎯 7 种预设计划模板** ✨ NEW
  - 7 天睡眠改善 - 通过科学方法改善睡眠质量
  - 14 天梦境回忆增强 - 提升梦境回忆能力
  - 30 天清醒梦入门 - 学习清醒梦技巧
  - 21 天压力缓解 - 管理和缓解压力
  - 14 天创意启发 - 从梦境获取创意灵感
...
- [x] **✅ 12 种习惯类型** ✨ NEW
  - 记录梦境/规律作息/冥想练习/现实检查/梦境孵化
  - 晨间反思/感恩日记/屏幕时间限制/咖啡因限制
  - 运动/呼吸练习/自定义习惯
...
- [x] **⚡ 核心服务** ✨ NEW
  - `DreamAICoachService`: AI 教练核心服务（@ModelActor 并发安全）
  - 计划 CRUD 操作
  - 习惯管理和打卡
  - 干预生成和管理
  - 统计计算
...
**新增文件**:
- `DreamAICoachModels.swift` (~480 行) - AI 教练数据模型
- `DreamAICoachService.swift` (~650 行) - AI 教练核心服务
- `DreamAICoachView.swift` (~850 行) - AI 教练 UI 界面
- `DreamLogTests/DreamAICoachTests.swift` (~550 行) - 单元测试
...
│   ├── DreamSmartSuggestionsModels.swift     # 智能建议数据模型 ✨ NEW Phase 85

...
class DreamJournalExportService {
    ...
    private func drawCoverPage(context: UIGraphicsPDFRendererContext, dreamCount: Int) {
        ...
        ("Generated by DreamLog 🌙" as NSString).draw(in: footerRect, withAttributes: footerAttributes)
    }
    ...
    private func drawTableOfContents(context: UIGraphicsPDFRendererContext, dreams: [Dream]) {
        ...
        (language.tableOfContents as NSString).draw(in: titleRect, withAttributes: titleAttributes)
        ...
        for (index, dream) in dreams.prefix(15).enumerated() {
            ...
            (entryText as NSString).draw(in: entryRect, withAttributes: entryAttributes)
        ...
        if dreams.count > 15 {
            ...
            (moreText as NSString).draw(in: moreRect, withAttributes: moreAttributes)
        }
    }
    ...
    private func drawStatisticsPage(context: UIGraphicsPDFRendererContext, dreams: [Dream]) {
        ...
        (language.statistics as NSString).draw(in: titleRect, withAttributes: titleAttributes)
        ...
        (emotionTitle as NSString).draw(in: emotionTitleRect, withAttributes: emotionTitleAttributes)
        ...
        for (emotion, count) in sortedEmotions {
            ...
            ("\(emotion): \(count)" as NSString).draw(in: labelRect, withAttributes: labelAttributes)
    ...
    private func drawDreamPage(context: UIGraphicsPDFRendererContext, dream: Dream, pageNumber: Int) {
        ...
        ((dream.title.isEmpty ? untitledDream : dream.title) as NSString).draw(in: titleRect, withAttributes: titleAttributes)
        ...
        if config.includeTags && !dream.tags.isEmpty {
            ...
            (tagsText as NSString).draw(in: tagsRect, withAttributes: tagsAttributes)
            metaY += 25
        }
        ...
        if config.includeEmotions && !dream.emotions.isEmpty {
            ...
            (emotionsText as NSString).draw(in: emotionsRect, withAttributes: emotionsAttributes)
            metaY += 25
        }
        ...
        (indicatorsText as NSString).draw(in: indicatorsRect, withAttributes: indicatorsAttributes)
        ...
        (dream.content as NSString).draw(in: contentRect, withAttributes: contentAttributes)
        ...
        if let aiAnalysis = dream.aiAnalysis, !aiAnalysis.isEmpty {
            ...
            ("🧠 AI 解析" as NSString).draw(in: CGRect(x: margins.left + 10, y: aiY, width: 100, height: 20), withAttributes: aiTitleAttributes)
            ...
            (aiAnalysis as NSString).draw(in: CGRect(x: margins.left + 10, y: aiY + 25, width: bounds.width - margins.left - margins.right - 20, height: 50), withAttributes: aiContentAttributes)
        }
